...
public class SchemeToDerivationImp implements SchemeToDerivation {
    ...
    @Override
    public Distribution<String> transform(Distribution<String> schemeDist,
                                          String swar, List<String> ascent,
                                          List<String> descent) {

        Distribution<String> derDist = new Distribution<>();
        
        derDist = schemeDist.getValues()
                .stream()
                .map(digits -> {
                    return schemeDist.keyString(digits) + "-" + schemeDist.get(digits);
                }).map(entry -> {
